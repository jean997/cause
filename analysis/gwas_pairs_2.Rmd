...
2. Edit the `config.yaml` file. You may not need to make any changes but you may need to change the location of the LD files or change where you would like data stored. Leave `all_pairs: True` or, alternatively to only run LDL -> CAD and LDL -> Asthma change to
